	...
	if (codemirrorEl.value) {
		...
		codemirror.on('change', (doc, { origin }) => {
			...
			if (content === '') {
				...
				invalidInput.value = !isValidJSON(content);
				console.log('CHANGE: ', unref(isObjectLike), isValidJSON(content), unref(isObjectLike) && isValidJSON(content) ?parseJSON(content) : content)
...
<template>
	<div class="system-raw-editor" :class="{ disabled, 'multi-line': isMultiLine }">
		...
		<template v-if="invalidInput">
			<small>Invalid JSON/JavasScript</small>
		</template>
	</div>
</template>
